#
# https://github.com/M3RG-IITD/LaMO/blob/main/model/Latent_SSM_unshared.py
import torch
...
class LaMOAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out_slice_token = self.ssm(slice_token)
        out_slice_token = rearrange(out_slice_token, 'b n (h d) -> b n h d', h=self.heads)
...
class LaMOBlock(nn.Module):
    ...
    def __init__(
        ...
        # self.Attn = LaMOAttention(hidden_dim, heads=num_heads, dim_head=hidden_dim // num_heads, dropout=dropout, slice_num=slice_num)
        self.Attn = Latent_SSM_Irregular_Mesh(hidden_dim, heads=num_heads, dim_head=hidden_dim // num_heads, dropout=dropout, slice_num=slice_num)
...
#======================================================================#
class LaMOAttention_Structured_Mesh_2D(nn.Module):
    ## for structured mesh in 2D space
...
class LaMOBlock_Structured_Mesh_2D(nn.Module):
    ...
    def __init__(
        ...
        self.last_layer = last_layer
        self.ln_1 = nn.LayerNorm(hidden_dim)
        # self.Attn = LaMOAttention_Structured_Mesh_2D(hidden_dim, heads=num_heads, dim_head=hidden_dim // num_heads, dropout=dropout, slice_num=slice_num, H=H, W=W)
        self.Attn = Latent_SSM_Structured_Mesh_2D(hidden_dim, heads=num_heads, dim_head=hidden_dim // num_heads, dropout=dropout, slice_num=slice_num, H=H, W=W)
...
class LaMO_Structured_Mesh_2D(nn.Module):
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([LaMOBlock_Structured_Mesh_2D(num_heads=n_head, hidden_dim=n_hidden,
                                                      dropout=dropout,
                                                      act=act,
                                                      mlp_ratio=mlp_ratio,
                                                      out_dim=out_dim,
